...
io.sockets.on('connection', (socket) => {
    ...
    socket.on('connectID', (previousID) => {
        console.log('-- Checking if this user is reconnecting to a running game...')
        ...
        if (Users.size > 0) {
            ...
            if (DCs.length > 0) {
                let matchedDCs = DCs.filter((DC) => DC.id == previousID)
                console.log("Does user match old socketid? " + user);
                if (matchedDCs.length > 0) {
                    console.log("new user matches old user's socketid: " + matchedDCs[0].id + "new users socket is " + socket.id);
                    ...
                    console.log("Disconnected list has found no matches");
                }
            } else {
                console.log("Reconnect Protocol -- No recent disconnects");
                addUser(socket);
            }
        } else {
            console.log("Adding New User:" + socket.id);
    ...
    socket.on('startGame', () => {
        ...
        if (playerCount < 1 || playerCount > 6) {
            ...
            for (user in socket.adapter.rooms[Users.get(socket.id).room].sockets) {
                changeUserPropertyWithID(user, 'state', states.GAME);
            }
    ...
    function beginDelayedRemoveUser(socket) {
        if (Users.has(socket.id)) {
            ...
            // checkUsers();
            // change playerPropertyToConnected = false ************
            // Games[Users.get(socket.id)['room']].Players.changePlayerPropertyWithID(socket.id, "connected", false);
...
class Game {
    ...
    updateReconnection(socketid) {
        ...
        // if FirstOutHappened
        if (Games[Users.get(socketid)['room']].OutPlayer > -1) {
            ...
            // send outdeck          Games[Users.get(socket.id)['room']].
            console.log("outdeck: " + Games[Users.get(socketid)['room']].OutDeck);
            // LEAH the socket event is 'updateOutDeck'
        ...
        // re-send round info
        io.to(socketid).emit('currentRound', { 'round': this.Round });
        ...
        // resend discard card
        let discardCard = this.DiscardPile[this.DiscardPile.length -1];
        io.to(socketid).emit('addToDiscard', { 'card': discardCard });

        // change Users and Players status to ROOM and ready
        this.changePlayerPropertyWithID(socketid, 'connected', true);
    ...
    setDeck() {
        this.Deck = ['Joker_0', 'Joker_0', 'Joker_0', 'Joker_0',
        ...
        this.DiscardPile = [];   // draw one card to discard pile
    ...
    drawCard() {
        ...
        let swap = this.Deck[rand];
        this.Deck[rand] = this.Deck[this.Deck.length - 1];
        this.Deck[this.Deck.length - 1] = swap;

        return this.Deck.pop();
    }

    discardToDeck() {
        let topOfDiscard = this.DiscardPile.pop();
        let secondOfDiscard = this.DiscardPile.pop();

        this.DiscardPile.forEach((value) => {
            this.Deck.push(value);
        });

        this.DiscardPile = [secondOfDiscard, topOfDiscard];

        console.table(this.DiscardPile);
